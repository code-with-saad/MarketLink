      ...
      <div className="space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="border-earth-200/80 bg-warm-surface p-6 space-y-3">
            ...
            <h3 className="font-serif text-lg font-bold text-foreground">Guaranteed Pickup Slots</h3>
          ...
          <Card className="border-earth-200/80 bg-warm-surface p-6 space-y-3">
            ...
            <h3 className="font-serif text-lg font-bold text-foreground">100% Local Growers</h3>
            <p className="text-sm text-earth-700 leading-relaxed">
          ...
          <Card className="border-earth-200/80 bg-warm-surface p-6 space-y-3">
            ...
            <h3 className="font-serif text-lg font-bold text-foreground">Direct Community Impact</h3>
            <p className="text-sm text-earth-700 leading-relaxed">
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-earth-200/80 pb-4">
            <div>
              <span className="text-xs font-bold uppercase tracking-widest text-primary">Marketplace Showcase</span>
              <h2 className="font-serif text-2xl sm:text-3xl font-bold text-foreground mt-1">Featured Harvest Items</h2>
            </div>
            <Link to="/customer/products" className="text-sm font-bold text-primary hover:opacity-80 flex items-center gap-1.5 group">
          ...
          {isLoading ? (
              ...
              {products.map((item) => (
                <Card key={item._id || item.id} className="overflow-hidden flex flex-col group">
                  <div className="relative h-48 bg-earth-100 overflow-hidden">
                    {item.image_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-warm-cream-dark text-earth-500">
                        <FontAwesomeIcon icon={faLeaf} className="text-3xl text-primary/40" />
                      </div>
                    )}
                  ...
                  <CardFooter className="p-5 pt-3 border-t border-earth-100 flex items-center justify-between mt-auto">
                    <div>
                      <span className="font-serif text-xl font-extrabold text-foreground">${item.price}</span>
            ...
            <Card className="p-12 text-center bg-warm-surface border-dashed border-earth-300 space-y-3">
              <div className="w-12 h-12 rounded-2xl bg-warm-cream-dark text-primary flex items-center justify-center text-xl mx-auto">
                <FontAwesomeIcon icon={faLeaf} />
              </div>
              <h3 className="font-serif text-lg font-bold text-foreground">New listings coming soon</h3>
